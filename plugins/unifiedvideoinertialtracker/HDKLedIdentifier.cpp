...
namespace vbtracker {
    ...
    ZeroBasedBeaconId OsvrHdkLedIdentifier::detectPattern(ZeroBasedBeaconId currentId, uint16_t bits)
    {
        // No pattern recognized and we should have recognized one, so return
        // a low negative.  We've used -2 so return -3.
        ZeroBasedBeaconId id = Led::SENTINEL_NO_PATTERN_RECOGNIZED_DESPITE_SUFFICIENT_DATA;
        if (!bits)
            return id;
        ...
        if (isInSync()) {
            ...
            for (int i = 0; i < d_length; i++) {
                ...
                if (it != d_patterns.end()) {
                    ...
                    id = ZeroBasedBeaconId(it - d_patterns.begin());
                    break;
                }
